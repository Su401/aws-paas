...
export default function RegistarProdutos() {
	...
	const [caixaInputValidadeAberto, setCaixaInputValidadeAberto] =
		useState('');
	...
	let dataFormatadaPadrao = `${dataAtual.getFullYear()}-${
		dataAtual.getMonth() + 1
	}-${dataAtual.getDate()}`;
	const [caixaDataAbertura, setCaixaDataAbertura] =
		useState(dataFormatadaPadrao);

	const fetchDadosTiposProduto = async () => {
		try {
			const response = await fetch(
				'http://localhost:8080/api/registoDeProdutos/preencheProdutos',
				{
					method: 'GET',
					headers: {
			...
			setCaixaInputValidadeAberto(
				selectTypeData.allProducts[0].produtosArray[0].validade
			);
			setValorCaixaSelectNome(
				selectTypeData.allProducts[0].produtosArray[0].nomeProduto
			);
		} catch (error) {
	...
	const handleSelectTypeChange = (e) => {
		...
		const tipoProduto = dadosTiposProduto.filter(
			(Tipo) => Tipo.tipo_produto === novoValor
		);
	...
	const handleSelectNameChangeThroughTypeChange = (e) => {
		...
		const novoArray = dadosTiposProduto.find(
			(dados) => dados.tipo_produto === atualTipo
		);
	...
	const handleSelectNameChange = (e) => {
		const filtrarPorCaixaTipo = dadosTiposProduto.filter(
			(Tipo) => Tipo.tipo_produto === caixaSelectValor
		);
		const atualProduto = filtrarPorCaixaTipo[0].produtosArray.find(
			(produto) => produto.nomeProduto === e.target.value
		);
	...
	const handleBtnClick = () => {
		...
		novaData.setDate(
			dataAtual.getDate() + Number(caixaInputValidadeAberto)
		);
		dataFormatadaPadrao = `${novaData.getFullYear()}-${
			novaData.getMonth() + 1
		}-${novaData.getDate()}`;
		...
		if (
			caixaSelectValor !== '' &&
			valorCaixaSelectNome !== '' &&
			caixaInputLote !== '' &&
			caixaInputValidadeAberto !== '' &&
			caixaInputValidade !== ''
		) {
			const impressao = {
			...
			<Container className='container mt-5'>
				<Row className='row'>
					<Col className='col-12 col-lg-8 p-0 m-0'>
							...
							<Col className='col-md-4 col-12'>
								<div className='selection-box'>
									<select
										...
										value={caixaSelectValor}
										onChange={(e) => {
											handleSelectTypeChange(e);
											handleSelectNameChangeThroughTypeChange(
												e
											);
										}}
									>
										{dadosTiposProduto.map(
											(tipoProduto) => (
												<option
													value={
						...
						<Row className='row'>
							<Col className='col-md-4 col-12'>
								<div className='text-box text-center'>Lote</div>
							</Col>
							<Col className='col-md-4 col-12'>
								<div className='form-group text-center'>
									<input
										...
										min='0'
										value={caixaInputLote}
										onChange={(e) =>
											setCaixaInputLote(e.target.value)
										}
									></input>
								</div>
							</Col>
						...
						<Row className='row'>
							...
							<Col className='col-md-4 col-12'>
								<div className='form-group text-center'>
									<input
										type='text'
										name='productValidityAfterOpenned'
										id='productValidityAfterOpenned'
										className='form-input-group w-100 mt-1 text-center'
										value={Number(caixaInputValidadeAberto)}
									></input>
								</div>
							</Col>
						</Row>
							...
							<Col className='col-md-4 col-12'>
								<div className='form-group text-center'>
									<input
										...
										id='productValidity'
										className='form-input-group w-100 mt-1 text-center'
										value={caixaInputValidade}
										onChange={(e) =>
											setCaixaValidade(e.target.value)
										}
									></input>
								</div>
							</Col>
