...
export default function GerirProdutos() {
	...
	const fetchDados = async () => {
		try {
			const response = await fetch(
				'http://localhost:8080/api/criarTabela',
				{
					method: 'GET',
					headers: {
	...
	const updateProduct = async () => {
		try {
			...
			const response = await fetch(
				`http://localhost:8080/api/editarProduto/${productData.id}`,
				{
					method: 'PUT',
					headers: {
						'Content-Type': 'application/json',
	...
	const deleteProduct = async () => {
		try {
			const id = idInput;
			const response = await fetch(
				`http://localhost:8080/api/excluirProduto/${id}`,
				{
					method: 'DELETE',
					headers: {
						'Content-Type': 'application/json',
	...
	const addProduct = async () => {
		try {
			...
			const response = await fetch(
				`http://localhost:8080/api/adicionarProdutos`,
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
				...
				<Row>
					<Col lg={6} xs={12}>
						...
						<Table
							responsive
							id='tableProducts'
							className='table-container products-table table table-hover table-light table-sm align-middle'
						>
							<thead>
								<tr>
					...
					<Container className='col-lg-6 col-rgt'>
						...
						>
							<Row className='row div-product'>
								...
								<Col className='col-md-6 col-12'>
									<input
										className='product filled'
										type='text'
										id='product-type'
										value={tipoProdutoInput}
										onChange={(e) =>
											setTipoProdutoInput(e.target.value)
										}
									></input>
								</Col>
							</Row>
							<Row className='row div-product'>
								...
								<Col className='col-md-6 col-12'>
									<input
										...
										type='text'
										id='product-name'
										value={nomeProdutoInput}
										onChange={(e) =>
											setNomeProdutoInput(e.target.value)
										}
									></input>
								</Col>
							</Row>
							<Row className='row div-product'>
								...
								<Col className='col-md-6 col-12'>
									<input
										...
										value={validadeInput}
										onChange={(e) =>
											setValidadeInput(e.target.value)
										}
									></input>
								</Col>
							</Row>
						...
						<Row className='row mt-3'>
							...
							<Col className='col-md-6 col-12 col-btn'>
								<a href='#'>
									<button
										id='btnExcluirProdutos'
										className='btn btn-danger shadow-sm btn-lg'
										type='button'
