...
export default function PrintProfileModal(props) {
	...
	function handlePrintOrSend() {
		// Logic for printing or sending the content
		window.print(); // This will trigger the browser's print dialog
		// Alternatively, you can add logic for sending the content via email or API
	}
	...
	return (
		<div className='PrintProfileModal'>
			...
			>
						...
						<Container className='flex-container paperContainer sheet'>
							...
							<Row>
								...
								<p className='text'>
									...
									<i>{props.user.userAddress}</i>, e
									atualmente com o cargo de{' '}
									<i>{props.user.role}</i> no estabelecimento
									(preciso buscar mais info ), é assegurado
									pela <i>{props.user.insuranceName}</i> e tem
									o número de apólice:{' '}
									<i>{props.user.insurancePolicy}</i>.
								</p>
								&nbsp;
							</Row>
				...
				<Modal.Footer>
					<Button onClick={handlePrintOrSend}>Print/send</Button>
				</Modal.Footer>
			</Modal>
		</div>
	);
